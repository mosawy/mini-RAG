    ...
    if process_request.file_id:
        ...
        if asset_record is None:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "segnal":ResponceSegnal.FILE_ID_ERROR.value
                }
            )
        ...
        project_file_ids = {record.id : record.asset_name for record in project_file}
    
    if len(project_file_ids) == 0:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "segnal":ResponceSegnal.NO_FILES_ERROR.value
            }
        )
